 ...
 * @author THINKPAD
 */
public class DataCredentials extends javax.swing.JDialog {
    ...
    public String roles;
    public DataCredentials(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        setTableModel();
        loadData();
    ...
    private void setTableModel() {
        ...
        model.addColumn("ID Credentials");
    ...
    private void getData(DefaultTableModel model) {
        ...
        Root<Credentials> mhs = cq.from(Credentials.class);

        cq.multiselect(mhs.get("idCred"), mhs.get("username"), mhs.get("roles"));
        CriteriaQuery<Credentials> select = cq.select(mhs);
        TypedQuery<Credentials> q = em.createQuery(select);
        List<Credentials> list = q.getResultList();

        for (Credentials m : list) {
    ...
    public void cariData() {
        String[] valueModel = {"idCred", "username", "roles"};
        ...
        em.getTransaction().begin();
        CriteriaBuilder cb = em.getCriteriaBuilder();
        AbstractQuery<Credentials> cq = cb.createQuery(Credentials.class);

        Root<Credentials> Credentials = cq.from(Credentials.class);
        cq.where(cb.like(cb.upper(Credentials.get(valueModel[jComboBoxCari.getSelectedIndex()])), "%" + textFieldCari.getText().toUpperCase() + "%"));
//        cq.where(cb.equal(Credentials.get(valueModel[jComboBoxCari.getSelectedIndex()]), jTextFieldCari.getText()));
        CriteriaQuery<Credentials> select = ((CriteriaQuery<Credentials>) cq).select(Credentials);
        TypedQuery<Credentials> q = em.createQuery(select);
        List<Credentials> list = q.getResultList();
    ...
    private void pilihData() {
        ...
        this.idCred = jTable1.getValueAt(row, 0).toString();
        this.username = jTable1.getValueAt(row, 1).toString();
        this.roles = jTable1.getValueAt(row, 2).toString();
        dispose();
    }
    ...
    private void initComponents() {
        ...
        jComboBoxCari.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "ID", "Username", "Roles" }));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(DataCredentials.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                DataCredentials dialog = new DataCredentials(new javax.swing.JFrame(), true);
